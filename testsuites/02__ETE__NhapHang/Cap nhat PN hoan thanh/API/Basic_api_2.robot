...
ena02
    ...
    ${get_no_ncc_hientai}    ${get_tong_mua}    ${get_tong_mua_tru_tra_hang}    Get Supplier Debt from API    ${supplier_code}
    #
    Log    tinh tien hang
    ...
    #
    Log    tinh tong no, tong mua sau khi nhap hang
    ${result_no_phieunhap}    Minus    ${result_cantrancc}    ${input_datrancc}
    ...
    ...    ${result_ggpn_type}    ${get_id_pn}    ${result_cantrancc}    ${input_datrancc}
    Log    ${request_payload}
    ...
    Should Be Equal As Strings    ${get_supplier_code}    ${supplier_code}
    Should Be Equal As Numbers    ${get_tongtienhang}    ${result_tongtienhang}
    Should Be Equal As Numbers    ${get_tiendatra_ncc}    ${input_datrancc}
    ...
    \    ${item_imei}     Run Keyword If    '${item_status}'=='True'    Convert string to list      ${item_imei}    ELSE        Set Variable    ${item_imei}
    ...
    ${nav_tientra_ncc}=    Catenate    -    ${input_datrancc}
    ${get_ma_pn_soquy}    Get Ma Phieu Chi in So quy    ${receipt_code}
    Run Keyword If    '${input_datrancc}' == '0'    Validate So quy info from Rest API if Invoice is not paid    ${receipt_code}
    ...
    Run Keyword If    '${input_datrancc}'=='0'    Validate status in Tab No can tra NCC if purchase order is not paid until success    ${supplier_code}    ${receipt_code}
    ...    ELSE    Validate status in Tab No can tra NCC if purchase order is paid until success    ${supplier_code}    ${get_ma_pn_soquy}    ${receipt_code}
    Assert ma phieu nhap is avaliable in Tab No can tra NCC and Lich su nhap/tra hang    ${supplier_code}    ${receipt_code}
    #
    Log    validate tổng nợ, tổng mua
    ${get_no_af_purchase}    ${get_tongmua_af_purchase}    ${get_tongmua_tru_trahang_af_purchase}    Get Supplier Debt from API after purchase    ${supplier_code}    ${receipt_code}    ${input_datrancc}
