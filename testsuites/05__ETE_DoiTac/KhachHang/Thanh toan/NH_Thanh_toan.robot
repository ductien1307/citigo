...
*** Variables ***
&{list_prs_num}    GHU0006=7
@{list_giamoi}    80000


*** Test Cases ***    Mã KH         List products and nums    List giá mới      GGHD    Mã Combo    Khách thanh toán    TIền thu từ khách     Phương thức       Tài khoản NH
Thanh toan NH           [Tags]        CNKH
                      [Template]    Thanh toan cong no NH
                      CTKH005      ${list_prs_num}           ${list_giamoi}    20       none     100000              120000                    Thẻ                1234
                      CTKH005      ${list_prs_num}           ${list_giamoi}    10       none     80000                all                    Chuyển khoản         2134

*** Keywords ***
Thanh toan cong no NH
    ...
    ${result_tien_thu}    Set Variable If    '${input_tien_thu}'=='all'    ${get_no_cuoi_kh}    ${input_tien_thu}
    ...
    Input Text    ${textbox_thu_tu_khach_in_customer}    ${result_tien_thu}
    Select payment method and bank account in popup Thanh toan    ${phuong_thuc}        ${tk_ngan_hang}
