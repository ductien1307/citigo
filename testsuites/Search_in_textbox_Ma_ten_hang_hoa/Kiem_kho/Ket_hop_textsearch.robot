...
*** Test Cases ***    Tim theo              Text search                            Ma_sp     Ten sp                      Input_Search
hh da co trong phieu kiem kho
                      [Template]            Check product searching in kiem kho
                      Theo mã phiếu kiểm    KK456789                               SP0126    Khăn lụa giấy Kleenex $x    @{input_data_search}
                      Theo Serial/IMEI

*** Keywords ***
Check product searching in kiem kho
    [Arguments]    ${input_timtheo}    ${input_textsearch}    ${input_ma_sp}    ${input_ten_sp}    @{input_data_search}
    [Documentation]    *hh thường*..Check: Hàng hóa thường, đang kinh doanh, dưới định mức tồn, còn hàng trong kho
    [Timeout]
    Go to Kiem kho
    Sleep    2s
    Select and input data to textsearch    ${input_timtheo}    ${input_textsearch}
    #loop
