...
*** Test Cases ***    Start_date    End_date                                  Ma_sp     Ten sp                      Input_Search
hh da can bang kho    [Template]    Kiem kho with Toan thoi gian selection
                      1             31                                        SP0126    Khăn lụa giấy Kleenex $x    @{input_data_search}

*** Keywords ***
Kiem kho with Toan thoi gian selection
    [Arguments]    ${input_startdate}    ${input_enddate}    ${input_ma_sp}    ${input_ten_sp}    @{input_data_search}
    [Documentation]    *hh thường*..Check: Hàng hóa thường, đang kinh doanh, dưới định mức tồn, còn hàng trong kho
    [Timeout]
    Go to Kiem kho
    Sleep    2s
    Select start-end date    ${input_startdate}    ${input_enddate}
    #loop
