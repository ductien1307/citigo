...
50 dong               [Tags]                     DTKL
                      [Template]                 edthn_nn_02
                      ${list_product_nums_TH}    KLT0005                      50              10                   20000               150000
...
edthn_nn_02
    [Arguments]    ${dic_product_nums_th}    ${input_product_dth}    ${total_row}    ${input_phi_th}    ${input_ggdth}    ${input_khtt}
    #get data frm Trả hàng
    ${list_product_th}    Get Dictionary Keys    ${dic_product_nums_th}
    ${list_nums_th}    Get Dictionary Values    ${dic_product_nums_th}
    ${list_result_thanhtien_th}    ${list_giavon_th}    ${list_result_toncuoi_th}    Get list total sale - endingstock - cost frm product api    ${list_product_th}    ${list_nums_th}
    #get data form doi hang
    ${get_product}    ${get_baseprice}    Get product name and price frm API    ${input_product_dth}
    ${result_row}    Sum    ${total_row}    1
    ${thanhtien}    Multiplication and round    ${get_baseprice}    1
    ${result_tongtienmua}    Multiplication and round    ${thanhtien}    ${result_row}
    #input product into DTH form
    Wait Until Keyword Succeeds    3 times    8 s    Before Test turning on display mode    ${toggle_item_themdong}
    Select Return without Invoice from BH page
    Log    input from tra hang
    ...
    \    ...    ${item_nums}    ${lastest_num}    ${cell_laster_numbers_return}
    Log    input form doi hang
    Wait Until Keyword Succeeds    3 times    10 s    Input data in textbox and wait until it is visible    ${textbox_th_search_hangdoi}    ${input_product_dth}    ${cell_dth_sanpham}
    ...    ${cell_dth_ma_sanpham}
