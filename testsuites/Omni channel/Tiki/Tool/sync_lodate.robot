...
*** Test Cases ***      Tên shop tiki       SP       SL      ${ma_hh_tiki}         Giá bán             Giảm gía      Point             Tên KH        Địa chỉ           Thành phố     Khu vực         Phường xã             Điện thoại    Trạng thái
Sync order lodate         [Tags]              TIKIO
                        [Template]    sync_tiki6
                        [Timeout]     20 minutes
                        KiotViet          LDTK01     4       07070       ${list_giaban}     ${list_gg}      ${list_point}    Test kh tiki      1B Yết Kiêu     Hà Nội     Quận Hoàn Kiếm    Phường Trần Hưng Đạo     none      ${list_tiki1}
                        KiotViet          LDTK01     4       07070      ${list_giaban}     ${list_gg}      ${list_point}    Test kh tiki      1B Yết Kiêu     Hà Nội     Quận Hoàn Kiếm    Phường Trần Hưng Đạo     none      ${list_tiki2}

*** Keywords ***
sync_tiki6
    [Arguments]    ${input_shop_tiki}     ${ma_hh}     ${num}   ${ma_hh_tiki}   ${list_price}     ${list_discount}       ${list_discount_point}   ${ten_kh}     ${dia_chi}    ${thanh_pho}      ${khu_vuc}      ${phuong_xa}    ${dien_thoai}     ${list_trangthai_tiki}
    #
    Delete product if product is visible thr API    ${ma_hh}
    Add lodate product thr API    ${ma_hh}    test sync lodate   Mỹ phẩm     55000
    Mapping product with tiki thr API    ${input_shop_tiki}     ${ma_hh_tiki}    ${ma_hh}
