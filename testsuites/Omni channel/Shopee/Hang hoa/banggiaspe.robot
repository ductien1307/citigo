...
*** Test Cases ***      Tên shop Shopee  Tên bảng giá      Mã hh
Liên kết hh trong bảng giá chọn
                        [Tags]           SPEH
                        [Template]    mapapispe7
                        [Timeout]     3 minutes
                        thanhptk       Bảng giá Shopee     DVTKM02      SP000561

Update công thức bảng giá
                        [Tags]           SPEH
                        [Template]    mapapispe8
                        [Timeout]     3 minutes
                        thanhptk       Bảng giá Shopee     PIB10015      20

Update giá của bảng giá phụ thuộc
                        [Tags]           SPEH
                        [Template]    mapapispe9
                        [Timeout]     3 minutes
                        thanhptk      Bảng giá Shopee   PIB10015      10000

Thêm hh vào bảng giá
                        [Tags]           SPEH
                        [Template]    mapapispe10
                        [Timeout]     3 minutes
                        thanhptk      Bảng giá Shopee   TP231      10000

Xóa hh khỏi bảng giá
                        [Tags]           SPEH
                        [Template]    mapapispe11
                        [Timeout]     3 minutes
                        thanhptk      Bảng giá Shopee   TP231      10000

*** Keywords ***
mapapispe7
    [Arguments]   ${shop_shopee}   ${input_banggia}   ${ma_hh}     ${input_ma_hh_shopee}
    Delete product mapping Shopee if mapping is visible thr API    ${shop_shopee}   ${ma_hh}
    ...
    Mapping product with shopee thr API     ${shop_shopee}   ${input_ma_hh_shopee}    ${ma_hh}
    Log    validate lstt
    ${list_audit_db}     Create list audit trail for sync product       ${ma_hh}   ${gia_ban}   none
    Wait Until Keyword Succeeds    10x    1s    Assert audit trail by time succeed   ${get_cur_date}    ${ma_hh}    Đồng bộ Shopee    ${list_audit_db}
    Delete product mapping shopee thr API    ${shop_shopee}    ${ma_hh}

mapapispe8
    [Arguments]   ${shop_shopee}   ${input_banggia}   ${ma_hh}     ${input_percent}
...
mapapispe9
    [Arguments]   ${shop_shopee}   ${input_banggia}   ${ma_hh}     ${input_vnd}
    ...
    Wait Until Keyword Succeeds    90x    1s   Assert audit trail by time succeed   ${get_cur_date}    ${ma_hh}    Đồng bộ Shopee    ${list_audit_db}
