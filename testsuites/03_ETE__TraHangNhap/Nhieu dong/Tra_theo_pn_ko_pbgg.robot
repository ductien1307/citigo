...
*** Variables ***
@{list_sp}        NHMQD003        NHMS003       NHMT003
...
Ko phan bo gghd
    ...
    [Template]    etnd2
    [Timeout]     15 minutes
    [Documentation]   Trả hàng nhập theo phiếu nhập nhiều dòng CÓ phân bổ giảm giá > check chi tiêt đơn, tồn kho, giá vốn, công nợ nhà cung câp
    NCC0013    ${list_sp}    ${list_sl}    ${list_gm}    ${list_gg}    50000    30000     ${list_sl_tra}    all

*** Keywords ***
etnd2
    [Arguments]    ${input_supplier_code}    ${list_products}    ${list_num}    ${list_newprice}    ${list_discount}    ${input_discount_nh}
    ...    ${input_datrancc}    ${list_nums_return}    ${input_paid_supplier}
    ...
    ${list_result_thanhtien_af}     ${list_result_onhand_cb_af}    ${list_actual_num_cb}    Get list of total purchase return by receipt - result onhand actual product in case of multi row product    ${list_pr_cb}    ${list_products}
    ...    ${list_nums_return}    ${list_nh_result_newprice}
    ...
    ${list_cost_cb_af_ex}    Computation list of cost incase purchase return multi row    ${list_products}    ${list_pr_cb}    ${list_nh_result_newprice}    0    ${result_discount_thn}
    ...    ${result_tongtienhang}    ${list_nums_return}
