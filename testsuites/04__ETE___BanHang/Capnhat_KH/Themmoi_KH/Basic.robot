...
*** Variables ***
&{discount_1}      discount=0
&{discount_type1}      discounttype=none
&{product_type1}      protype=pro

*** Test Cases ***    Product Type       Product Discount      Discount Type        Invoice Discount      Payment
UI_Nor_unit_serial       [Tags]                               UPC
                      [Template]              upcus
                      ${product_type1}          ${discount_1}         ${discount_type1}       20          40000

*** Keywords ***
upcus
    [Arguments]    ${dict_product_type}      ${dict_discount}      ${dict_discount_type}      ${input_invoice_discount}    ${input_bh_khachtt}
    ${customer_code}      Create new Customer by generated info automatically
    ...
    ${get_no_bf_execute}    ${get_tongban_bf_execute}    ${get_tongban_tru_trahang_bf_execute}    Get Customer Debt from API    ${customer_code}
    ...
    Run Keyword If    '${input_bh_khachtt}' == 'all'    Input Khach Hang    ${customer_code}
    ...    ELSE    Input Invoice info    ${customer_code}    ${input_bh_khachtt}    ${result_khachcantra}
    ...
    Should Be Equal As Strings    ${get_ma_kh_by_hd}    ${customer_code}
    ...
    Run Keyword If    '${input_bh_khachtt}' == '0'    Validate status in Tab No can thu tu khach if Invoice is not paid until success    ${customer_code}    ${invoice_code}
    ...    ELSE    Validate status in Tab No can thu tu khach if Invoice is paid until success    ${customer_code}    ${get_maphieu_soquy}    ${invoice_code}
    Log        assert values in Khach hang and So quy
    ${get_no_af_purchase}    ${get_tongban_af_purchase}    ${get_tongban_tru_trahang_af_purchase}    Get Customer Debt from API after purchase    ${customer_code}    ${invoice_code}    ${actual_khachtt}
    ...
    Delete customer by Customer Code    ${customer_code}
