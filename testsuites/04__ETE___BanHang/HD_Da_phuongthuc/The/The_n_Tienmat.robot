...
*** Variables ***
&{invoice_1}      ComboPM38=4
&{discount_1}      ComboPM38=5
&{discount_type1}      ComboPM38=disvnd
&{product_type1}      ComboPM38=com

*** Test Cases ***    Product and num list         Product Type       Product Discount      Discount Type        Invoice Discount    Customer     Card Payment     Cash Payment
UI_Tienmat_n_The       [Tags]               PM
                      [Template]              card_n_cash_payment
                      ${invoice_1}            ${product_type1}          ${discount_1}         ${discount_type1}          0                   KH041       100000          50000

*** Keywords ***
card_n_cash_payment
    [Arguments]    ${dict_product_num}    ${dict_product_type}      ${dict_discount}      ${dict_discount_type}      ${input_invoice_discount}    ${input_bh_ma_kh}    ${input_card_payment}       ${input_cash_payment}
    [Timeout]
    ...
    ${actual_khachtt}    Sum    ${input_card_payment}    ${input_cash_payment}
    ...
    Run Keyword If    '${input_card_payment}' == 'all'    Input Khach Hang    ${input_bh_ma_kh}
    ...    ELSE    Input Invoice info    ${input_bh_ma_kh}    ${input_card_payment}    ${result_khachcantra}
    Go to popup for other payment methods
    ${khachtt_forinput}       Convert To String    ${actual_khachtt}
    Apply Card    ${input_card_payment}
    Apply Cash       ${input_cash_payment}
    ...
    Run Keyword If    '${input_card_payment}' == '0'    Validate status in Tab No can thu tu khach if Invoice is not paid until success    ${input_bh_ma_kh}    ${invoice_code}
    ...
    Run Keyword If    '${input_card_payment}' == '0'    Validate So quy info from Rest API if Invoice is not paid until success    ${get_maphieu_soquy}
    ...    ELSE    Validate So quy info from Rest API if Invoice is paid in case multi-payments    ${get_maphieu_soquy}    ${input_cash_payment}     ${input_card_payment}
