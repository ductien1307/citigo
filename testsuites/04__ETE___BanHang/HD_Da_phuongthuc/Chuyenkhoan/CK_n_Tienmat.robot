...
&{invoice_1}      IM21=2
&{discount_1}      IM21=5
&{discount_type1}      IM21=dis
&{product_type1}      IM21=imei

*** Test Cases ***    Product and num list         Product Type       Product Discount      Discount Type        Invoice Discount    Customer     Transfer Payment     Cash Payment
UI_Tienmat_n_CK       [Tags]               PM
                      [Template]              transfer_n_cash_payment
                      ${invoice_1}            ${product_type1}          ${discount_1}         ${discount_type1}          0                   KH039       100000          50000

*** Keywords ***
transfer_n_cash_payment
    [Arguments]    ${dict_product_num}    ${dict_product_type}      ${dict_discount}      ${dict_discount_type}      ${input_invoice_discount}    ${input_bh_ma_kh}    ${input_transfer_payment}       ${input_cash_payment}
    [Timeout]
    ...
    ${actual_khachtt}    Sum    ${input_transfer_payment}    ${input_cash_payment}
    ...
    Run Keyword If    '${input_transfer_payment}' == 'all'    Input Khach Hang    ${input_bh_ma_kh}
    ...    ELSE    Input Invoice info    ${input_bh_ma_kh}    ${input_transfer_payment}    ${result_khachcantra}
    Go to popup for other payment methods
    ${khachtt_forinput}       Convert To String    ${actual_khachtt}
    Apply Transfer    ${input_transfer_payment}
    ...
    ...    ELSE    Validate So quy info from Rest API if Invoice is paid in case multi-payments    ${get_maphieu_soquy}    ${input_cash_payment}     ${input_transfer_payment}
