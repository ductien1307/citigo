...
*** Variables ***
&{invoice_1}      DV037=10       DVT0215=1
&{discount_1}      DV037=50000       DVT0215=0
&{discount_type1}      DV037=disvnd       DVT0215=changedown
&{invoice1_product_type}      DV037=ser      DVT0215=pro
&{invoice1_promo_type}      DV037=promo      DVT0215=getpromo
...
KM HD_HH_Tang          [Tags]             ACBPPI
                      [Template]              akm_inv_prod_14
                      ${invoice_1}            ${invoice1_product_type}          ${discount_1}         ${discount_type1}          5         5       KH033        all            KM014          ${invoice1_promo_type}           #KM này bị lỗi khi post qua API ko hiển thị đúng
...
akm_inv_prod_14
    ...
    ${invoice_value}    ${received_value}    ${discount}    ${discount_ratio}    ${name}    ${promotion_sale_id}       ${promotion_id}     Get Invoice value - Received value - Discounts - Promotion Name - Promotion Id - Promotion Sale Id from Promotion Code    ${input_khuyemmai}
    ...
    ${first_quan}       Get From Dictionary       ${dict_product_num}         DV037
    ${text_promo_info}        Format String      {0}: Tổng tiền hàng từ {3} và mua {1} DV037 - Đính đá (Viên) tặng 1 DVT0215 - Kẹo Alpenliebe Hương Dâu Kem cho hóa đơn       ${name}      ${first_quan}        ${actual_text_discount}        ${text_total_invoice}
