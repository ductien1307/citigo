...
*** Variables ***
&{invoice_1}      PIB10036=1
&{discount_1}      PIB10036=6500000
&{discount_type1}      PIB10036=changeup
&{product_type1}      PIB10036=pro

*** Test Cases ***    Product and num list    Product Discount                                                    Invoice Discount    Customer    Payment    Promotion Code         Surcharge Code
KM HD_HH_GiamgiaHD          [Tags]           CONPROTK12           UEB1         GOLIVE1_A
                      [Template]              con_km1213_tk0307
                      ${invoice_1}            ${product_type1}          ${discount_1}         ${discount_type1}          5                   KH034        0            KM012          TK003

*** Keywords ***
con_km1213_tk0307
    [Arguments]    ${dict_product_num}    ${dict_product_type}      ${dict_discount}      ${dict_discount_type}      ${input_invoice_discount}    ${input_bh_ma_kh}    ${input_bh_khachtt}     ${input_khuyemmai}       ${input_thukhac}
    Log      Thu khac
    ...
    ${result_discount_invoice_by_vnd}    Run Keyword If    0 < ${input_invoice_discount} < 100    Convert % discount to VND and round    ${result_total_invoice}    ${input_invoice_discount}
    ...    ELSE    Set Variable    ${input_invoice_discount}
    ${actual_promo_discount_value}=    Run Keyword If    ${discount_ratio} != 0    Convert % discount to VND and round    ${result_total_invoice}    ${discount_ratio}      ELSE     Set Variable    ${discount}
    ${final_discount_incl_promo}=    Run Keyword If    ${result_total_invoice} > ${invoice_value}    Sum    ${actual_promo_discount_value}    ${result_discount_invoice_by_vnd}
    ...    ELSE    Set Variable    ${result_discount_invoice_by_vnd}
    ${result_total_invoice_af_inv_n_promo_discount}    Minus    ${result_total_invoice}    ${final_discount_incl_promo}
    ${actual_surcharge_value}    Run Keyword If    ${surcharge_%} != 0.0    Convert % discount to VND and round    ${result_total_invoice_af_inv_n_promo_discount}    ${surcharge_%}      ELSE       Set Variable     ${surcharge_vnd_value}
    ${result_total_invoice_incl_surcharge}    Minus    ${result_total_invoice}    ${actual_surcharge_value}
    ${result_khachcantra_incl_surcharge}        Sum        ${result_total_invoice}       ${actual_surcharge_value}
    ${result_khachcantra}    Minus    ${result_khachcantra_incl_surcharge}    ${final_discount_incl_promo}
    ...
    Run Keyword If    ${result_total_invoice} > ${invoice_value}    Wait Until Keyword Succeeds    3 times    8 s    Select promotion    ${name}
    ...
    ...    ${final_discount_incl_promo}
    ...    ELSE IF    ${input_invoice_discount} > 100    Input VND discount invoice incase promotion discount    ${input_invoice_discount}       ${final_discount_incl_promo}
    ...
    #Assert Invoice until success    ${invoice_code}    ${result_khachcantra_incl_surcharge}    ${result_khachcantra}    ${actual_khachtt}    ${input_bh_ma_kh}    ${final_discount_incl_promo}
    ...
    #...    ${invoice_code}
    #Should Be Equal As Numbers    ${get_tong_tien_hang}    ${result_khachcantra_incl_surcharge}
    ...
    #Should Be Equal As Numbers    ${get_giamgia_hd}    ${final_discount_incl_promo}
