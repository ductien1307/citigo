...
ForEach ($tag in $tags) {
	...
	Write-Host "----------------------------"
	pabot --processes $parallel -d $report/$tag -r $report_file --variable env:$env --variable remote:$remote --variable account:$account --variable headless_browser:$headless_browser -i $tag prepare
	Write-Host "==================================================================================================="
}
