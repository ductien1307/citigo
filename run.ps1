...
ForEach ($tag in $tags) {
	...
	Write-Host ""
	pabot --processes $parallel -T -d reports/$tag --variable env:$env --variable remote:$remote --variable account:$account --variable headless_browser:$headless_browser -i $tag testsuites
	Write-Host "==================================================================================================="
}
